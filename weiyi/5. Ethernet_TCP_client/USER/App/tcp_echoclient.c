...
void tcp_vibration_send(u8 *data);
...
void tcp_temprature_send(void)
{
	...
	for(i=1;i<15;i++)
			{
				t_data[i] = g_tAD7606.tcpadc[i+1];
			}
			/* allocate pbuf */
      //echoclient_es->p_tx = pbuf_alloc(PBUF_TRANSPORT, strlen((char*)data) , PBUF_POOL);
			echoclient_es->p_tx = pbuf_alloc(PBUF_TRANSPORT, strlen((char*)t_data) , PBUF_POOL);
			//pbuf_take(echoclient_es->p_tx, (char*)data, strlen((char*)data));
			pbuf_take(echoclient_es->p_tx, (char*)t_data, strlen((char*)t_data));
...
void tcp_vibration_send(u8 *data)
{
			u8 length = strlen((char*)data);
      echoclient_es->p_tx = pbuf_alloc(PBUF_TRANSPORT, 1001, PBUF_POOL);
						//pbuf_take(echoclient_es->p_tx, (char*)data, strlen((char*)data));
			pbuf_take(echoclient_es->p_tx, (char*)data, 1001);
			tcp_echoclient_send(echoclient_pcb, echoclient_es);
			tcp_output(echoclient_pcb);
}
...
void tcp_pulse_send(void)
{
			...
			p_data[1] = time >> 8;
			p_data[0] = 'C';
      echoclient_es->p_tx = pbuf_alloc(PBUF_TRANSPORT, 3, PBUF_POOL);
						//pbuf_take(echoclient_es->p_tx, (char*)data, strlen((char*)data));
			pbuf_take(echoclient_es->p_tx, (char*)p_data, 3);
			tcp_echoclient_send(echoclient_pcb, echoclient_es);
			tcp_output(echoclient_pcb);
}
/**
