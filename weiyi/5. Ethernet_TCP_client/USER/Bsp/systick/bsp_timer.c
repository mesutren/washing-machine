...
void SysTick_ISR(void)
{
	...
	/* 全局运行时间每1ms增1 */
	LocalTime++;
	if (LocalTime == 0x7FFFFFFF)	/* 这个变量是 int32_t 类型，最大数为 0x7FFFFFFF */
	{
		LocalTime = 0;
	}
...
int32_t bsp_GetRunTime(void)
{
	...
	runtime = LocalTime;	/* 这个变量在Systick中断中被改写，因此需要关中断进行保护 */
